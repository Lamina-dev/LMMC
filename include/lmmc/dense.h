...
/**
 * @brief 分配并初始化一个 @p rows x @p cols 的稠密矩阵,所有元素置零.
 *
...
/**
 * @brief 创建 nxn 单位矩阵(对角线为 1,其余为 0).
 *
 ...
 * 当 @p tol <= 0 时,使用默认容差:max(rows, cols) * LMMC_REAL_EPSILON * sigma_0,
 * 其中 sigma_0 为最大奇异值.
 *
 * @param[in]  a        输入矩阵(不被修改).
 ...
 *
 * 内部分配两个 nxn 临时矩阵用于迭代,返回前释放.
 *
 * @param[in]  a   输入方阵(不被修改).
 * @param[in]  n   整数指数,可为负,零或正.
 * @param[out] out 输出矩阵,须已创建且与 @p a 同维(nxn).
 *
 * @return
 * - ::LMMC_STATUS_OK - 成功.
 ...
 *
 * 要求 A 为方阵(nxn),B 的列数等于 A 的行数(即 B 为 mxn),
 * X 的维度为 mxn.
 *
 * @param[in]  B 被除矩阵(mxn,不被修改).
 * @param[in]  A 除数方阵(nxn,不被修改).
 * @param[out] X 输出矩阵(mxn),须已创建且维度为 (B->rows x A->cols).
 *
 * @return
 * - ::LMMC_STATUS_OK - 成功.

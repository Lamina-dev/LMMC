 ...
 */
#ifndef LMMC_STD_STDLIB_H
#define LMMC_STD_STDLIB_H
...
/** @brief std.linalg.eig 使用的 LSR 规范的 table<text, matrix> 视图. */
typedef struct {
...
/** @brief std.linalg.svd 使用的 LSR 规范的 table<text, matrix> 视图. */
typedef struct {
    lmmc_mat_t U;
    lmmc_mat_t S;
    lmmc_mat_t Vt;
...
/** @brief 按索引返回 LSR 规范的 std.constants 条目名;越界索引映射为 NULL. */
...
/** @brief 按名称返回 LSR 规范的 std.constants 单位字符串;无单位条目映射为 NULL. */
const char* lmmc_std_constants_unit(const char* name);
/** @brief 按索引返回完整的 LSR 规范的 std.constants 条目. */

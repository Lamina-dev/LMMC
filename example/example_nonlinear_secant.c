/**
 * @file example_nonlinear_secant.c
 * @brief 演示 LMMC 中 nonlinear secant 相关接口的使用。
 */
#include <stdio.h>
#include "lmmc/lmmc.h"
...
int main(void) {
    ...
    st = lmmc_secant_solve(lmmc_example_fn, NULL, 1.0, 2.0, &cfg, &result);
    if (st != LMMC_STATUS_OK) {
        printf("secant failed: %s\n", lmmc_status_string(st));
        return 1;
    }

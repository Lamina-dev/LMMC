...
int main(void) {
    ...
    TEST_ASSERT(lmmc_eps(&val) == LMMC_STATUS_OK && val == DBL_EPSILON);
    ...
    // Enhanced Trig
    TEST_ASSERT(lmmc_atan2(1.0, 1.0, &val) == LMMC_STATUS_OK && fabs(val - 0.7853981633974483) < 1e-10);
    TEST_ASSERT(lmmc_sincos(0.5, &val, &val2) == LMMC_STATUS_OK && fabs(val - sin(0.5)) < 1e-15 && fabs(val2 - cos(0.5)) < 1e-15);
    TEST_ASSERT(lmmc_hypot(3.0, 4.0, &val) == LMMC_STATUS_OK && fabs(val - 5.0) < 1e-15);

    // Power and Log
    TEST_ASSERT(lmmc_exp2(3.0, &val) == LMMC_STATUS_OK && fabs(val - 8.0) < 1e-15);
    TEST_ASSERT(lmmc_log2(8.0, &val) == LMMC_STATUS_OK && fabs(val - 3.0) < 1e-15);

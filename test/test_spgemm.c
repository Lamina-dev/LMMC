...
int main(void) {
    ...
    LMMC_REAL_SET_D(&a_dense.data[0], 1.0); LMMC_REAL_SET_D(&a_dense.data[1], 0.0); LMMC_REAL_SET_D(&a_dense.data[2], 2.0);
    LMMC_REAL_SET_D(&a_dense.data[3], 0.0); LMMC_REAL_SET_D(&a_dense.data[4], 3.0); LMMC_REAL_SET_D(&a_dense.data[5], 0.0);

    st = lmmc_mat_create(3, 2, &b_dense);
    if (st != LMMC_STATUS_OK) { rc = 1; goto cleanup; }
    LMMC_REAL_SET_D(&b_dense.data[0], 0.0); LMMC_REAL_SET_D(&b_dense.data[1], 4.0);
    LMMC_REAL_SET_D(&b_dense.data[2], 5.0); LMMC_REAL_SET_D(&b_dense.data[3], 0.0);
    LMMC_REAL_SET_D(&b_dense.data[4], 0.0); LMMC_REAL_SET_D(&b_dense.data[5], 6.0);
    ...
    if (st != LMMC_STATUS_OK) { rc = 1; goto cleanup_eye; }
    LMMC_REAL_SET_D(&eye_dense.data[0], 1.0); LMMC_REAL_SET_D(&eye_dense.data[4], 1.0); LMMC_REAL_SET_D(&eye_dense.data[8], 1.0);
    st = lmmc_sparse_from_dense(&eye_dense, 1e-14, &eye);
    if (st != LMMC_STATUS_OK) { rc = 1; goto cleanup_eye; }

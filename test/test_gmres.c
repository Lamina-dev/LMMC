/**
 * @file test_gmres.c
 * @brief 针对 LMMC 中 gmres 相关接口的单元测试。
 *
 * @internal
 */
#include <math.h>
...
int main(void) {
    ...
    if (!lmmc_test_nearly_equal(x.data[0], 1.0, 1e-6) ||
        !lmmc_test_nearly_equal(x.data[1], 2.0, 1e-6) ||
        !lmmc_test_nearly_equal(x.data[2], 3.0, 1e-6)) {
        rc = 1;
        goto cleanup;
    }

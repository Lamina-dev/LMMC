...
static char* lmmc_std_text_copy(const char* value)
{
...
static int lmmc_std_text_set_contains_value(const lmmc_std_text_set_t* set,
                                            const char* value)
{
    for (size_t i = 0; i < set->size; ++i) {
...
static lmmc_status_t lmmc_std_text_set_alloc(size_t capacity,
...
static lmmc_status_t lmmc_std_text_set_append_unique(
    lmmc_std_text_set_t* set,
    const char* value)
{
    char* copy = NULL;
...
lmmc_status_t lmmc_std_text_set_subset(const lmmc_std_text_set_t* lhs,
                                       ...
                                       int* out)
{
    if (!out || !lmmc_std_text_set_valid(lhs) ||
        !lmmc_std_text_set_valid(rhs)) {
        return LMMC_STATUS_INVALID_ARGUMENT;
    }
...
static lmmc_status_t lmmc_std_text_set_binary_alloc(
    ...
    lmmc_std_text_set_t* out)
{
    if (!out || (const void*)out == (const void*)lhs ||
        ...
        !lmmc_std_text_set_valid(lhs) || !lmmc_std_text_set_valid(rhs)) {
        return LMMC_STATUS_INVALID_ARGUMENT;
    }
    if (lhs->size > ((size_t)-1) - rhs->size) {
        return LMMC_STATUS_INVALID_ARGUMENT;
    }
    return lmmc_std_text_set_alloc(lhs->size + rhs->size, out);
}

#ifndef LMMC_LSR_STDLIB_INTERNAL_H
#define LMMC_LSR_STDLIB_INTERNAL_H

/* Private shared helpers for the lsr_stdlib translation units. */
#include <math.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
static inline lmmc_status_t lmmc_lsr_store_real(lmmc_real_t value, lmmc_real_t* out)
{
...
static inline lmmc_status_t lmmc_lsr_store_finite_real(lmmc_real_t value,
                                                lmmc_real_t* out)
{
    if (!out) return LMMC_STATUS_INVALID_ARGUMENT;
...
static inline int lmmc_lsr_real_is_finite(lmmc_real_t value)
{
    return isfinite((double)value);
}

static inline int lmmc_lsr_real_array_is_finite(const lmmc_real_t* values,
                                         size_t count)
{
...
static inline int lmmc_lsr_mul_overflow_size(size_t a, size_t b, size_t* out)
{
    if (a == 0 || b == 0) {
        *out = 0;
        return 0;
...
static inline int lmmc_lsr_complex_is_finite(const lmmc_complex_t* z)
{
    return z && lmmc_lsr_real_is_finite(z->real) &&
           lmmc_lsr_real_is_finite(z->imag);
}

static inline uint64_t lmmc_lsr_hash_mix64(uint64_t value)
{
    value ^= value >> 30;
...
static inline uint64_t lmmc_lsr_real_hash_bits(lmmc_real_t value)
{
    uint64_t bits = 0;
...
static inline int lmmc_lsr_bool_is_valid(int value)
{
    return value == 0 || value == 1;
}

static inline uint64_t lmmc_lsr_text_hash_bytes(const char* value)
{
...
static inline int lmmc_lsr_mat_valid(const lmmc_mat_t* a)
{
    return a && a->data && a->rows > 0 && a->cols > 0 && a->stride >= a->cols;
}

static inline int lmmc_lsr_mat_is_finite(const lmmc_mat_t* a)
{
...
static inline lmmc_status_t lmmc_lsr_require_finite_mat(const lmmc_mat_t* a)
{
...
static inline int lmmc_lsr_vec_is_finite(const lmmc_vec_t* v)
{
    return v && v->data && v->size > 0 &&
           lmmc_lsr_real_array_is_finite(v->data, v->size);
}

static inline lmmc_status_t lmmc_lsr_require_finite_vec(const lmmc_vec_t* v)
{
...
static inline int lmmc_lsr_compare_eval(lmmc_real_t lhs,
...
static inline lmmc_status_t lmmc_lsr_bool_vec_create(size_t size,
...
static inline lmmc_status_t lmmc_lsr_bool_mat_create(size_t rows,
                                              size_t cols,
...
static inline lmmc_status_t lmmc_lsr_require_finite_eig_result(
...
static inline lmmc_status_t lmmc_lsr_require_finite_svd_result(
...
static inline lmmc_status_t lmmc_lsr_copy_mat(const lmmc_mat_t* src, lmmc_mat_t* out)
{
    lmmc_status_t status;
...
static inline lmmc_status_t lmmc_lsr_copy_vec(const lmmc_vec_t* src, lmmc_vec_t* out)
{
    lmmc_status_t status;
    if (!out) return LMMC_STATUS_INVALID_ARGUMENT;
...
static inline int lmmc_lsr_real_is_integer(lmmc_real_t value)
{
    double as_double = (double)value;
    return isfinite(as_double) && floor(as_double) == as_double;
}

static inline lmmc_status_t lmmc_lsr_pow_finite(lmmc_real_t base,
...
static inline lmmc_status_t lmmc_lsr_vec_to_column(const lmmc_vec_t* src,
                                            lmmc_mat_t* out)
{
    lmmc_status_t status;
...
static inline lmmc_status_t lmmc_lsr_vec_to_diag(const lmmc_vec_t* src,
                                          size_t rows,
                                          size_t cols,
                                          lmmc_mat_t* out)
{
...
static inline lmmc_real_t lmmc_lsr_num_set_key(lmmc_real_t value)
{
